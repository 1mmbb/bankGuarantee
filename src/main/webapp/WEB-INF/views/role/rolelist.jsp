  ...
  <tr>
    ...
    <td>
    	<form action="" method="post">
    		<input id="inputname" name="rolename" type="text" size="30" placeholder="角色名称..."/>
    		<input type="button" id="search" value="查 询" class="btn" onmouseover="changebg(this,'1')" onmouseout="changebg(this,'1')" />
    		<a href="/role/toAddRole" style="text-decoration:none">
	  ...
	  <c:if test="${!empty pi.list }">
	  	<c:forEach items="${pi.list}" var="item">
	  		<tr>
				...
				<td style="text-align:center;">
					<a href="/role/toEditRole?roleid=${item.roleid}">编辑</a>
					<a href="/role/toRoleRight?roleid=${item.roleid}">权限</a>
				</td>
			</tr>
		</c:forEach>
	  </c:if>
			...
			$.ajax({  
                ...
                success:function(data){  
                    ...
                    for(var i=0;i<li.length;i++){  
                        ...
                        str+='<td style="text-align:center;">'+li[i].roledesc+'</td>';  
                        if(li[i].rolestate==1){
							str+='<td style="text-align:center;">正常</td>'; 
						}else{
							str+='<td style="text-align:center;">锁定</td>';
						} 
                        str+='<td style="text-align:center;"><a href=\"/role/toEditRole?roleid='+li[i].roleid+'\">编辑</a>'; 
                        str+='<a href=\"/role/toRoleRight?roleid='+li[i].roleid+'\">权限</a></td>';
                        str+="</tr>";  
